...
import { signup, sendMagicLink, login } from '../lib/mock-auth';
...
export function SignupPage({ onNavigate, onSignupSuccess }: SignupPageProps) {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        // In mock mode, auto-login after signup
        const loginResult = await login(email, 'mock-password');
        if (loginResult.success) {
          toast.success('Account created! Welcome to XOVA.');
          onSignupSuccess();
        } else {
          setMagicLinkSent(true);
          toast.success('Magic link sent! Check your email to get started.');
        }
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-xova-primary/3 via-xova-accent/3 to-white flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <Card className="w-full max-w-md p-8 relative shadow-xl border-xova-primary/20">
        ...
        <p className="text-muted-foreground text-center mb-8">Join XOVA and start your health journey</p>
            ...
            {magicLinkSent ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground mb-6">
                  Click the link in your email to complete your signup and log in. It may take a few minutes to arrive.
                </p>
                <Button
                  variant="outline"
